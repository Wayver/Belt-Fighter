...
def draw_hud(screen, font, score, wave, enemies, ship):
    ...
    screen.blit(font.render(f"WAVE {wave}", True, (120, 200, 255)), (8, 8 + HUD_ROW))
    screen.blit(font.render(f"ENEMIES {len(enemies)}", True, ENEMY_EDGE), (8, 8 + 2 * HUD_ROW))
    screen.blit(font.render(f"v={ship.vel.length():.0f}", True, (110, 120, 140)),
                (WIDTH - 70, HEIGHT - 22))
    y = 8 + 3 * HUD_ROW + 8
    _bar(screen, font, 8, y, "PWR", ship.power_used, ship.power_supply,
         warn=ship.brownout)
    _bar(screen, font, 8, y + HUD_ROW, "CPU", ship.compute_used, ship.compute_supply)
    if ship.shield_comp is not None:
        _bar(screen, font, 8, y + 2 * HUD_ROW, "SHD", ship.shield_charge,
             ship.shield_comp.shield_max_charge, warn=not ship.shield_on)
    if ship.targeting_on:
...
def _bar(screen, font, x, y, label, used, supply, warn=False):
    text = font.render(f"{label} {used:.0f}/{supply:.0f}", True, (200, 210, 225))
    screen.blit(text, (x, y))
    bx, by, bw, bh = x + text.get_width() + 10, y + 4, 140, 12
